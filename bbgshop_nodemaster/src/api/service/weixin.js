...
module.exports = class extends think.Service {
  ...
  createUnifiedOrder(payInfo) {
    console.log("进入api的createUnifiedOrder事件");
    ...
    return new Promise((resolve, reject) => {
      weixinpay.createUnifiedOrder({
        ...
        if (res.return_code === 'SUCCESS' && res.result_code === 'SUCCESS') {
          ...
          const paramStr = `appId=${returnParams.appid}&nonceStr=${returnParams.nonceStr}&
          package=${returnParams.package}&signType=${returnParams.signType}&
          timeStamp=${returnParams.timeStamp}&key=` + think.config('weixin.partner_key');
          returnParams.paySign = md5(paramStr).toUpperCase();
          resolve(returnParams);
        } else {
          reject(res);
